    ...
    <div className=''>
      <nav className="bg-bgColor py-2 lg:py-0 fixed w-full z-50 top-0 left-0 border-gray-200 dark:bg-gray-900 ">
        ...
        <div className="flex items-center md:order-2 gap-3 px-4 relative">
            ...
            <button onClick={toggleLanguageMenu} type="button" className="inline-flex items-center font-medium bg-gray-100 justify-center px-4 py-2 text-sm text-gray-900 dark:text-white rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-white">
            ...
            <div className="gap-3 justify-between hidden lg:flex py-2 px-7">
              {
                ...
                (
                  <>
                    <NavLink onClick={closeMobileMenu} to='/login' className='flex justify-center border-2 border-awimGreen text-awimGreen text-md hover:bg-awimGreen hover:text-textWhite items-center py-2 px-8 transition duration-300 ease-in-out rounded-lg'>Login </NavLink>
                    <NavLink onClick={closeMobileMenu} to='/register' className='flex justify-center border-2 border-awimGreen bg-awimGreen hover:bg-bgColor hover:text-awimGreen hover:border-awimGreen hover:border-2 text-bgColor items-center py-2 px-8 rounded-lg transition duration-300 ease-in-out'>Sign Up</NavLink>
                  </>
                )
              }
            ...
            <button onClick={toggleMobileMenu} aria-expanded={isMobileMenuOpen ? 'true' : 'false'}  type="button" className={`inline-flex items-center p-2 w-9 h-9 justify-center text-sm text-gray-500 rounded-lg lg:hidden bg-awimGreen focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-language transition-transform duration-300 ${isMobileMenuOpen ? 'transform rotate-180' : ''} `}>
              {
                !isMobileMenuOpen ? (
                  <svg width="18" height="12" viewBox="0 0 18 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect width="17.1429" height="1.71429" rx="0.857143" fill="white"/>
                    <rect y="5.14282" width="17.1429" height="1.71429" rx="0.857143" fill="white"/>
                    <rect y="10.2856" width="17.1429" height="1.71429" rx="0.857143" fill="white"/>
                  </svg>
                ) : (
                  <svg width="18" height="17" viewBox="0 0 18 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect width="21.1468" height="2.22816" rx="1.11408" transform="matrix(0.707109 -0.707104 0.707109 0.707104 0 14.9529)" fill="white"/>
                    <rect width="21.1468" height="2.22816" rx="1.11408" transform="matrix(0.707109 0.707104 -0.707109 0.707104 2.19714 0.0302734)" fill="white"/>
                  </svg>
                )
              }
        ...
        <div className={`items-center justify-between ${isMobileMenuOpen ? 'block' : 'hidden'} w-full bg-white rounded-b-2xl lg:flex lg:w-auto md:order-1 transition ease-in-out duration-300`} id="navbar-language">
          <ul className="flex flex-col gap-2 font-medium p-4 lg:p-0 mt-4 lg:flex-row lg:space-x-8 lg:mt-0 lg:border-0 lg:bg-white transition duration-300 ease-in-out">
            {
              
              Menu.map(oneMenu => (
                <li key={oneMenu.name}>
                  <NavLink onClick={closeMobileMenu} to={oneMenu.path} className={`block py-2 pl-3 pr-4 rounded md:bg-transparent hover:text-awimYellow transition duration-300 ease-in-out md:p-0 md:dark:text-blue-500 ${oneMenu.familyLink.includes(pathName) ? 'text-awimYellow' : 'text-gray-900' } `} aria-current="page">{oneMenu.name}</NavLink>
                </li>
              ))
            }
          ...
          <div className="flex justify-between lg:hidden py-2 px-7">
            {
              isLoggedIn ? (<LoggedInMobile user={userData} config={config} closeMobileMenu={closeMobileMenu} />) :
              <>
                <NavLink onClick={closeMobileMenu} to='/login' className='flex justify-center border-2 border-awimGreen text-awimGreen text-md hover:bg-awimGreen hover:text-textWhite items-center py-2.5 px-12 transition duration-300 ease-in-out rounded-lg'>Login </NavLink>
                <NavLink onClick={closeMobileMenu} to='/register' className='flex justify-center border-2 bg-awimGreen hover:bg-bgColor hover:text-awimGreen hover:border-awimGreen hover:border-2 text-bgColor items-center py-2.5 px-12 rounded-lg transition duration-300 ease-in-out'>Sign Up</NavLink>
              </>
            }
          </div>
