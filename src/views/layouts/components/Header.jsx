...
function Header({config}) {
  ...
  return (
    <div className=''>
      <nav className="bg-bgColor py-2 lg:py-0 fixed w-full z-20 top-0 left-0 border-gray-200 dark:bg-gray-900 ">
          ...
          <ul className="flex flex-col gap-2 font-medium p-4 lg:p-0 mt-4 lg:flex-row lg:space-x-8 lg:mt-0 lg:border-0 lg:bg-white transition duration-300 ease-in-out">
            {
              ...
              Menu.map(oneMenu => (
                <li key={oneMenu.name}>
                  <NavLink to={oneMenu.path} className={`block py-2 pl-3 pr-4 rounded md:bg-transparent hover:text-awimYellow transition duration-300 ease-in-out md:p-0 md:dark:text-blue-500 ${oneMenu.familyLink.includes(pathName) ? 'text-awimYellow' : 'text-gray-900' } `} aria-current="page">{oneMenu.name}</NavLink>
                </li>
              ))
            }
