    ...
    <div className={`${myclass} border-2 border-[#0F172A10] h-auto ${expert.availability ? 'rounded-r-3xl rounded-bl-3xl rounded-tl-lg' : 'rounded-3xl'} `}>
        ...
        <div className="middle auto py-7 px-4">
            ...
            <div className="top flex gap-3 items-center">
                <h2 className="text-xl font-medium">{expert.name}</h2>
                <HtmlRender htmlString={expert.countryIcon} />
            </div>

            <div className="title flex gap-3 pt-1 items-center">
              <div className="text-[#0F172A60]">{expert.role}</div>
            ...
            <div className="language pt-5">
              <p className="lang text-xs font-semibold">Interview Language: {expert.langaugaes}</p>
            </div>

            <div className="messageBtn pt-9">
              <button disabled={expert.availability ? 'false' : 'true'} className={`bg-awimGreen transition duration-300 ease-in-out border-2 border-awimGreen py-3 w-full text-textWhite font-medium text-sm rounded-xl flex items-center justify-center gap-3 hover:bg-transparent hover:border-awimGreen hover:text-awimGreen ${expert.availability ? '' : 'disabled:opacity-50 disabled:pointer-events-none'}`}>
